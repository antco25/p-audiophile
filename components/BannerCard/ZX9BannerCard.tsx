...
const ZX9BannerCard = ({ data, ...props }: BannerProps) => {
  ...
  return (
    <div className={`${props.className ? props.className + ' ' : ''}text-white h-[720px] lg:h-[560px]`}>
      <div className='bg-pOrange-200 h-full overflow-hidden rounded-lg flex flex-col lg:flex-row relative'>
        <div className='relative flex-1'>
          <PatternCircles className='absolute -top-[288px] lg:-top-[36px] right-2/4 lg:-right-[240px] translate-x-2/4 lg:translate-x-0' />
          {
            !loaded ? null :
              <img src={isDesktop ? data.imageDesktop : isTablet ? data.imageTablet : data.imageMobile}
                alt='Product Banner'
                className={`absolute top-[52px] lg:top-auto lg:-bottom-2 right-2/4 lg:right-11 translate-x-2/4 lg:translate-x-0 
                w-[170px] xs:w-[200px] lg:w-[23rem]`} />
          }
        </div>
        <div className='flex-1'>
          <div className='relative h-full flex flex-col justify-center z-10 items-center lg:items-start lg:ml-28 text-center lg:text-left'>
