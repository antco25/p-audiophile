...
//TODO: Remove item individually
const CartModal = () => {
  const { showCart, setShowCart, cartItems, removeAllCart, updateCartItem, totalPrice } = useStateContext();
      ...
      <div className={`${commonStyles.appWrap} h-full relative`}>
        <div className='absolute right-0 top-[130px] bg-white rounded-lg w-[377px] p-8' onClick={(e) => { e.stopPropagation() }}>
          <div className='flex mb-8'>
            ...
            <button className={`underline opacity-50 font-medium${cartItems.length === 0 ? ' invisible' : ''}`} onClick={handleRemoveAll}>Remove all</button>
          </div>
          ...
          <div className={`flex items-center mb-6${cartItems.length === 0 ? ' invisible' : ''}`}>
          ...
          {
            cartItems.length === 0 ?
              <button className={`${commonStyles.buttonLinkOne} text-white text-center w-full block`} disabled>Checkout</button> :
              <Link href='/checkout' className={`${commonStyles.buttonLinkOne} text-white text-center block`}
